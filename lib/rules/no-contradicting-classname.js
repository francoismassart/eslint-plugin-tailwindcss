    ...
    const scriptVisitor = {
      ...
      CallExpression: function (node) {
        ...
        node.arguments.forEach((arg) => {
          // Can't determine if peer keys will be applied together
          ...
          if (arg.type === 'ObjectExpression') {
            arg.properties.forEach((p) => {
              switch (p.type) {
                case 'Property':
                  parseForContradictingClassNames(p.value.split(' '), node);
                  break;
                case 'SpreadElement':
                  p.argument.properties.forEach((sp) => {
                    if (sp.type === 'Property') {
                      parseForContradictingClassNames(sp.key.value.split(' '), node);
                    }
                  });
                  break;
              }
