  ...
  create: function (context) {
    ...
    const sortNodeArgumentValue = (node, arg = null) => {
      ...
      if (originalClassNamesValue !== validatedClassNamesValue) {
        const singleLineEndPos = arg && arg['type'] === 'TemplateElement' ? 1 : 0;
        const multiLineEndPos = arg && arg['type'] === 'TemplateElement' ? 2 : 1;
        const endPos = isSingleLine ? singleLineEndPos : multiLineEndPos;
        context.report({
          node: node,
          messageId: 'invalidOrder',
          fix: function (fixer) {
            return fixer.replaceTextRange([start, end - endPos], validatedClassNamesValue);
          },
        });
      }
    };
