  ...
  create: function (context) {
    ...
    const sortNodeArgumentValue = (node, arg = null) => {
      ...
      if (officialSorting) {
        ...
        // Ordering based on groups and supporting more options
        if (removeDuplicates) {
        ...
        if (before !== null) {
          orderedClassNames.unshift(before);
        }
        if (after !== null) {
          orderedClassNames.push(after);
        }
      }
      let validatedClassNamesValue = orderedClassNames.join(isSingleLine ? ' ' : '\n');
